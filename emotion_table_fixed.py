...
def feat(p):
    # r.landmarks is a DataFrame with x_0..x_67 / y_0..y_67 columns, NOT a list.
    # Indexing it with [0] raises KeyError, which a broad except would swallow
    # and turn every call into a silent None -- that bug emptied the whole table
    # on the first run, so landmarks are extracted separately and defensively.
    try:
    ...
    try:
        ...
        pred=FM.get(str(max(em.index,key=lambda c:em[c])).lower())
    except Exception:
        return None,None,None
    lm=None
    try:
        row=r.landmarks.iloc[0]
        lm=np.stack([[row[f'x_{i}'] for i in range(68)],
                     [row[f'y_{i}'] for i in range(68)]],axis=1)
    except Exception:
        pass
    return au, pred, lm
...
for e in E:
    ...
    aus=[]
    for f in fs:
        # Real RAVDESS frames are small and MTCNN often returns no box on them.
        # py-feat runs its own detection and alignment before computing AUs, so
        # the reference profile is built from the raw photograph directly.
        a,_,_=feat(f)
...
import json as _j
_prev=_j.load(open("full_emotion_table.json"))
ours=_prev["ours"]; edtalk=_prev["edtalk"]; eat=_prev["eat"]; pdfgc=_prev["pdfgc"]
_skip=collect("ours",
...
def eat_get(e,cid):
    m=f"exps/images/eat/{e}/{cid}.mp4"
    if not os.path.exists(m): return None
    fr=midframe(m); return facecrop(fr,f"t_{e}_{cid}") if fr is not None else None
eat=collect("eat", eat_get, lambda cid: eat_get("neutral",cid))
def pd_get(e,cid):
    p=f"exps/images/pdfgc/{e}/{cid}.jpg"
    if not os.path.exists(p): return None
    return facecrop(Image.open(p).convert("RGB"), f"p_{e}_{cid}")
pdfgc=collect("pdfgc", pd_get, lambda cid: pd_get("neutral",cid))
def sc_get(e,cid):
    p=f"exps/images/styleclip/{e}/{cid}_edit.png"   # edited half only
    if not os.path.exists(p): return None
    return facecrop(Image.open(p).convert("RGB"), f"sc_{e}_{cid}")
styleclip=collect("styleclip", sc_get, lambda cid: sc_get("neutral",cid))
...
print("-"*71)
print(row("Ours",ours)); print(row("EDTalk",edtalk)); print(row("EAT",eat)); print(row("StyleCLIP",styleclip)); print(row("PD-FGC*",pdfgc))
print("-"*71)
...
json.dump({"ours":{k:[float(x) for x in v] for k,v in ours.items()},
           ...
           "eat":{k:[float(x) for x in v] for k,v in eat.items()},
           "pdfgc":{k:[float(x) for x in v] for k,v in pdfgc.items()},
           "styleclip":{k:[float(x) for x in v] for k,v in styleclip.items()}},
