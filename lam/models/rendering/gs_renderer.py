...
class GS3DRenderer(nn.Module):
    ...
    def forward_gs(self,
        ...
        debug: bool = False,
        color_style: Optional[tuple] = None,
        **kwargs):
        ...
        for b in range(batch_size):
            ...
            # Damp the lateral asymmetry the decoder invents in its own xyz
            # offsets. Measured with style and emotion both disabled: a
            # point's offset vs its mirror partner's disagrees by 58.4% of
            # offset magnitude over the head, 37.3% in the mouth region --
            # which is where the visible lip distortion comes from (rendered
            # lip shape asymmetry ~6.2 vs ~1.8 in the source photos). Acts
            # only on the learned residual, never on the FLAME surface
            # point, so the subject's real geometry is untouched.
            gs_symmetry = float(getattr(self, "gs_mouth_symmetry", 0.0))
            if gs_symmetry and getattr(ret_gs, "offset", None) is not None:
                from lam.models.motion_symmetry import symmetrize_gaussian_offsets
                new_offset, new_xyz = symmetrize_gaussian_offsets(
                    ret_gs.offset, query_points[b], self.flame_model, gs_symmetry,
                    mouth_only=bool(getattr(self, "gs_symmetry_mouth_only", True)),
                )
                ret_gs.offset = new_offset
                ret_gs.xyz = new_xyz

            gs_model_list.append(ret_gs)

        if color_style is not None:
            # Applied to the decoded RGB color only, after xyz/scaling/rotation/opacity
            # have already been computed from the (unperturbed) shared hidden features.
            # Style used to modulate gs_hidden_features itself before this decode, which
            # meant "appearance" changes leaked into Gaussian scale/opacity and blurred
            # the render -- isolating it here to color-only fixes that at the source.
            color_gamma, color_beta = color_style
            for b, gs_model in enumerate(gs_model_list):
                # color_gamma/beta are per query-point: [B, N_points, 3]. query
                # points map 1:1 to Gaussians here (skip_decoder path), so
                # gamma_b/beta_b align with gs_model.shs row-for-row; add the
                # singleton dim shs itself carries ([N, 1, 3]) to broadcast.
                gamma_b, beta_b = color_gamma[b], color_beta[b]
                if gamma_b.dim() == 2:
                    gamma_b = gamma_b[:, None, :]
                    beta_b = beta_b[:, None, :]
                styled_shs = gs_model.shs * (1.0 + gamma_b) + beta_b
                if self.gs_net.use_rgb:
                    # shs holds sigmoid-activated RGB in [0, 1] in this mode; full SH
                    # coefficients (use_rgb=False) have no such range to clamp to.
                    styled_shs = torch.clamp(styled_shs, 0.0, 1.0)
                gs_model.update_shs(styled_shs)
