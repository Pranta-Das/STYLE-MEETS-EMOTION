...
class FlameTrackingSingleImage:
    ...
    def preprocess(self, input_image_path):
        ...
        # Landmark detection. FaceBoxes is trained on real photos (WIDER
        # FACE), so it under-scores stylized/illustrated art -- confirmed on
        # an AAHQ cel-shaded illustration that scored 0.8+ on every other
        # tested style image but only 0.23 here, well below the 0.8
        # threshold, despite the box itself being a correct, well-placed
        # face detection (verified by eye). Retry once at a much lower
        # threshold rather than raising the default outright, since fully
        # lowering it for every call would also let genuine false positives
        # on real photos through -- this fallback only ever fires when the
        # strict pass found nothing, and logs loudly so a bad box is easy to
        # spot rather than silently accepted.
        detections, _ = self.detector.detect(saved_image, 0.8, 1)
        if len(detections) == 0:
            detections, _ = self.detector.detect(saved_image, 0.2, 1)
            if len(detections) > 0:
                logger.warning(
                    f'No face detected at confidence>=0.8; falling back to a '
                    f'low-confidence detection ({detections[0][1]:.2f}) -- '
                    f'verify the result looks right.')
        if len(detections) == 0:
        ...
        if face_landmarks is None:
            logger.error('Failed to compute face landmarks')
            return ERROR_CODE['FailedToDetect']
