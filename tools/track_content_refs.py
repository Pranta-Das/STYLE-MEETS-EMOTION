...
def _select_candidates(paths, cfg):
    paths = sorted(paths)
    max_track_images = int(cfg.get("max_track_images", 300))
...
def track_content_refs(cfg, dry_run=False):
    output_dir = cfg.get("output_dir", "exps/train_lam/content_refs")
    os.makedirs(output_dir, exist_ok=True)

    tracking_output_dir = cfg.get("tracking_output_dir", "tracking_output")
    candidate_paths = sorted(glob.glob(cfg.candidate_content_glob))
    selected_paths = _select_candidates(candidate_paths, cfg)
    skip_existing = bool(cfg.get("skip_existing_tracking", True))

    summary = {
        "stage": "track_content_refs",
        "candidate_content_glob": cfg.candidate_content_glob,
    ...
    if not selected_paths:
        summary["error"] = "No candidate content images found."
        summary_path = os.path.join(output_dir, "tracking_summary.json")
        with open(summary_path, "w") as f:
            json.dump(summary, f, indent=2)
        raise FileNotFoundError(f"No images matched {cfg.candidate_content_glob}")
    ...
    if dry_run:
        for index, image_path in enumerate(selected_paths, start=1):
            ...
            summary["items"].append({
                ...
                "content_id": _content_id(image_path),
    ...
    for index, image_path in enumerate(selected_paths, start=1):
        ...
        if skip_existing and os.path.exists(shape_path):
            ...
            print(f"[track-content] skip existing {image_path}")
            continue

        print(f"[track-content] {index}/{len(selected_paths)} {image_path}", flush=True)
        try:
    ...
    # Distilled manifest for the training dataloader: only usable records.
    manifest = [
        {
        ...
        if item["status"] in {"tracked", "skipped_existing"} and os.path.exists(item["shape_path"])
    ]
    manifest_path = os.path.join(output_dir, "tracked_content_refs.json")
    with open(manifest_path, "w") as f:
        json.dump(manifest, f, indent=2)

    print(f"tracking_summary={summary_path}")
    print(f"tracked_content_refs={manifest_path}")
...
def main():
    parser = argparse.ArgumentParser(description="FLAME-track a CelebA-HQ content subset for style-adapter training.")
    parser.add_argument("--config", default="configs/train/track_content_refs_celeba.yaml")
    parser.add_argument("--dry-run", action="store_true", help="Only write the selected candidate list.")
    args = parser.parse_args()
    cfg = OmegaConf.load(args.config)
    track_content_refs(cfg, dry_run=args.dry_run)
