...
def demo_lam(flametracking, lam, cfg):
    ...
    def core_fn(image_path: str, video_params, working_dir, enable_oac_file):
        ...
        style_image_path = cfg.get("style_image_path", "./style_image.png")
        style_track_geometry = cfg.get("style_track_geometry", True)
        style_opacity_strength = cfg.get("style_opacity_strength", 0.45)
        ...
        # See lam/runners/infer/lam.py: the LAB post-process must measure the
        # style's tracked head crop, not the raw style file.
        style_ref_rgb = None
        ...
        with torch.no_grad():
            ...
            res = lam.infer_single_view(image.unsqueeze(0).to(device, dtype), None, None, 
                                        ...
                                        style_appearance_strength=cfg.get("style_internal_appearance_strength", 0.75),
                                        style_geometry_strength=cfg.get("style_geometry_strength", 0.85),
                                        style_opacity_strength=style_opacity_strength)
        
        # save h5 rendering info
        if h5_rendering:
            h5_fd = "./runtime_data"
            lam.renderer.flame_model.save_h5_info(shape_param.unsqueeze(0).to(device), fd=h5_fd)
        ...
        if enable_oac_file:
            try:
                ...
                saved_head_path = lam.renderer.flame_model.save_shaped_mesh(shape_param.unsqueeze(0).to(device), fd=oac_dir)
        ...
        style_strength = cfg.get("style_strength", 0.75)
        ...
        Image.fromarray(rgb[0]).save(dump_image_path)
        if vis_motion:
            vis_ref_img = np.tile(
...
def launch_gradio_app():
    ...
    print("Model built, moving to device...")
    try:
        lam.to('cuda')
    except:
